...
import { Clock, Check, Volume2, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ReviewPage() {
  ...
  const totalPages = Math.max(1, Math.ceil(reviewWords.length / itemsPerPage));

  // Clamp current page when the list updates
  React.useEffect(() => {
    if (currentPage > totalPages) {
      setCurrentPage(totalPages);
    }
  }, [reviewWords.length, totalPages, currentPage]);
  ...
  const currentItems = reviewWords.slice(
